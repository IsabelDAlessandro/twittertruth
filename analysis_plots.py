...
yVals, predictionsTest, probabilitiesTest = returnVals()
...
for i in range(0, len(yVals)):
    for j in range(0, len(yVals[i])):
        confusionMat[indexMap[yVals[i][j]], indexMap[predictionsTest[i][j]]] += 1
...
print precision
print recall
print np.mean(precision) #73.22%
print np.mean(recall) #75.25%